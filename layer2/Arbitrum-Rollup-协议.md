...
让我们从基础开始。 虚拟机（VM）的状态被编码为一颗 Merkle 树，因此可以计算 VM 状态的加密哈希。 在协议的任何时候，VM 的某些状态都已被完全确认并处于最终状态。 这个状态哈希存储在链上。

协议的参与者可以提出一个有争议的声明（DA），其中包括在一定的技术先决条件下从某种状态哈希开始，VM 可以执行指定数量的计算步骤，从而得出指定的新状态哈希， 并且 VM 在该计算期间进行指定的付款并发出指定的日志事件。 DA 可能有效（即真实）或无效。 提出 DA 的一方将被要求在 DA 的有效性上提交押金。 （有关赌注及其运作方式的更多信息，请参见下文。）
