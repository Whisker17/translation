...
尽管区块链互操作性对于如今的技术而言是可能的，但在技术可行性和实现无缝互操作性的数字经济之间还有其他缺失的部分。首先，我们的互操作性方案只会产生更多的分裂网络。像 [Polkadot](https://polkadot.network/) 和 [Cosmos](https://cosmos.network/) 这样的项目已经定义了各自的标准，并试图围绕自己的 “中心” 建立一个多链网络。同时也在努力搭建比特币和以太坊之间的直连桥梁。不可思议的是，在未来的某个时候，这些独立网络的核心团队和社区会坐到谈判桌前，就一个通用的互操作性标准达成一致。与此同时，更重要的是，即使这些区块链网络能够完美地互操作，糟糕的互操作用户体验仍然会使客户避而远之。从用户的角度来看，如果我是一个比特币用户，想要将我的比特币转移到以太坊以参与 DeFi 应用程序，我必须首先运行我的比特币钱包，然后跨链，然后使用一个独立的以太坊钱包。为了完成这样的跨链操作，我必须安装两个钱包应用程序，保留两套助记词并使用两个地址。这个过程本身是非常难以控制的，只适用于两条链。随着用户想要与更多的区块链交互，他们需要管理的助记词/地址/密钥对就越多。这种用户体验问题不仅阻碍了 dapp 的使用，也破坏了区块链的核心价值，即去中心化，因为用户将被迫依赖中心化的托管服务，以避免所有的麻烦。

解决这两个问题需要一种新的互操作性，我们称之为互操作性 2.0 。具有这种新的互操作性的区块链将像一个 “通用中心” ，可以与任何其他区块链交互而不被其他区块链注意到。一个 “通用中心” 必须能够理解和执行其他区块链的协议，而不是发明自己的协议并要求其他人学习。一个通用中心将类似于一个会说其他语言的多语言个体，因此中心可以与每个人交谈，每个人都愿意与中心对话。在加密世界中，所有协议（即区块链使用的语言）都由密码学组成，这意味着这样一个通用中心必须支持我们今天和未来将看到的在区块链中使用的广泛的密码学原语。通用中心必须能够理解各种区块链钱包签署的交易，允许用户坚持使用一个单一的钱包，同时能够使用在通用中心上运行的任何应用程序。

互操作性 2.0 的这些要求（密码学原语和发送者身份验证）正是一种新的抽象所能提供的。这就是为什么现在[一个以太坊用户可以使用他/她的 Metamask 钱包在 Nervos CKB 上操作其资产或运行 dapps](https://talk.nervos.org/t/2-0-nervos-unidapp-dapp/5332) ，而不需要任何手动设置，甚至不需要知道他/她正在使用 Nervos 的应用程序。不仅以太坊用户， [EOS/Tron 等区块链的用户也可以在 Nervos CKB 上操作其资产或运行 dapp ](https://pay.lay2.dev/#/)。如果你发现你钟爱的区块链还没有出现在列表中，不用担心，你可以简单地为它创建和部署智能合约(也可以等待或者雇佣智能合约开发人员为你做这些)。你无需向核心团队交流或提出硬分叉，所有这些都可以通过智能合约完成。

对于运行在 Nervos 上的应用程序，他们可以免费获得互操作性 2.0 。因此， Nervos 应用程序可以被每个区块链社区的用户广泛访问，我们称之为**通用应用程序**。作为一名开发人员，通过学习如何在 Nervos 上构建应用程序，您可以接触到比其他地方更大的用户基础。作为一个用户，你所需要做的就是使用你当前的钱包和账户使用 Nervos 的通用应用程序，不需要安装或学习任何新的东西。你可能认为你正在使用以太坊或 EOS 的 DApp ，而底层的信道和基础设施已经被 Nervos 取代。我相信本就该是这样的情况。比如，访问网站的用户并不关心网站是用 PHP 还是 JAVA 编写的，使用的是 MySQL 还是 PostgreSQL 。用户根本不在乎，这是理所当然的。作为开发人员，我们有责任将其抽象，并向用户隐藏实现细节，这样我们就可以更好的进行实现的迭代，以获得更好的用户体验。互操作性 2.0 将使加密世界变得像如今的互联网一样，这是通过新的区块链抽象实现的。

![Screenshot_20210317_160247](https://raw.githubusercontent.com/Whisker17/ImageStoreService/main/img/20210321211234.jpg)

更好的是，通用应用程序可以被比区块链用户多得多的用户访问。加密世界是一个相对较小的圈子，我们可以走得更远。区块链钱包和帐户只不过是另一个帐户/身份系统。然而，在互联网世界中，存在许多已建立的身份/帐户和认证标准，如 OpenID 、人脸识别、指纹识别等。通过加密码学语和发送者身份验证的抽象， Nervos CKB 也可以和被广泛使用的互联网协议更好的兼容。用户可以通过浏览器和手机访问通用应用程序，不需要安装任何区块链钱包，生成任何密钥对，甚至不需要记住任何助记词。通过这种方式，我们迎合了现有的互联网生态系统，而不是创建一个全新的生态系统。我们不会要求自己的奶奶去学一些她自己都不知道的魔法。阻止互联网用户进入加密世界的障碍在这里将不复存在。

![Screenshot_20210317_160407](https://raw.githubusercontent.com/Whisker17/ImageStoreService/main/img/20210321211247.jpg)

## 一个新的大都市

许多现代大都市都是从贸易路线的枢纽或港口发展而来的。威尼斯、纽约、香港、上海和新加坡都是凭借港口而成为高度商业化的城市。在工业时代， GPS 、货船和集装箱是我们用来在城市之间转移资产的互操作技术，但现在我们有了加密资产、区块链和互操作 2.0 。更好的互操作性给城市带来了更多的移民、更多的企业和更多的活力。摩天大楼会拔地而起，货物会被运送，资产会被保留，人们会留下来，一个新的大都市会诞生。
